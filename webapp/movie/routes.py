...
def movie(movieId):
    ...
    try:
        ...
        abort(404, error)
    print(movie,tags,cast,series)
    #replace the url with embed in it
    #movie.video_link.replace('watch?v=','embed/')
    movie_dict = {}
    movie_dict['link'] = movie.video_link.replace('watch?v=','embed/')
    return render_template("movie_details.html", movie=movie, tags=tags, cast=cast, series=series,movie_dict=movie_dict)
